...
namespace ZCFrame
{
    ...
    /// 游戏对象孵化池
    /// </summary>
    internal class GameObjctSpawnPool
    {
       ...
       // private string m_PrefabPath = "";
        ...
        public GameObjctSpawnPool(ushort poolId)
        {
        ...
        public T Dequeue<T>(short prefabIndex) where T : GameObjectItem
        {
            lock (m_ClassObjectPoolDic)
            {
                ...
                if (!m_ClassObjectPoolDic.TryGetValue(prefabIndex, out queue))
                {
                    ...
                    {
                        Debug.LogError(string.Format("加载预制体 {0:s} 失败，预制体索引 {1:d} 超出界限 {2:d}", Pooldata.name,
                            prefabIndex, Pooldata.prefabCount));
                        return default(T);
                    }
        ...
        public void Enqueue<T>(T t) where T : GameObjectItem
        {
            lock (m_ClassObjectPoolDic)
            {
                ...
                else
                {
                    ...
                    Debug.LogError(string.Format("回池的类对象，不是从池中创建的，请检查，对象:{0:s}  对象编号{1:d}  对象预制体编号:{2:d}", t.name, t.Id,
                        t.PrefabId));
                }
            }
        }
        ...
        private T OnNeedCreateNewObject<T>(short prefabIndex) where T : GameObjectItem
        {
            ...
            {
                Debug.LogError(string.Format("创建对象池失败，对象类型:{0:s} 与 实际对象类型:{1:s} 不一致, 请检查对象池编号:{2:D}", typeNmae,
                    Pooldata.name, Pooldata.id));
                return default(T);
            }
            ...
            Debug.LogError(string.Format("未找到对象的预制体，请检查: {0:s}  对象池编号:{1:D}  预制体编号:{2:D}", typeof(T).Name, Pooldata.id,
                prefabIndex));
            return default(T);
        }
    }
